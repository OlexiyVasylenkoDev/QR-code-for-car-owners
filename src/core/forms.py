    ...
    def clean(self):
        ...
        try:
            if bool(cleaned_data["phone"]):
                if (
                    get_user_model()
                    .objects.filter(phone=cleaned_data["phone"])
                    .exists()
                ):
                ...
                raise ValidationError(
                    "Write down your phone number to proceed registration please."
                )
...
class CustomAuthenticationForm(AuthenticationForm):
    ...
    username = UsernameField(
        label="Phone", widget=forms.TextInput(attrs={"autofocus": True})
    )
